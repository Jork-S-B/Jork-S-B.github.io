...
1.准备项目环境: 服务器操作系统-Centos7.6、资源情况-4核CPU、8G内存、部署方式-Docker、压测网络-局域网

2.准备测试环境: 测试机操作系统-Windows10、测试工具-JMeter、堆内存小于(物理内存*0.7)
...
5.搭建监控平台: 服务器资源、组件(db、mq、redis)通过Prometheus、Grafana监控，具体的进程监控通过命令行查看
