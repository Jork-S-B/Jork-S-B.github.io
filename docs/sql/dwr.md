...
1. ODS 数据源入湖
2. DWD 明细清晰: 过滤、去重、ID转名称等
3. DWS 预聚合: 汇总结果转宽表，空间换时间
4. ads bi报表展示，以视图或结果表的形式避免复杂运算。

### Q&A

Q: 表A有日期-YYYYMMDD、金额字段，写SQL：汇总 1 月份总额，计算 2 月环比（（2 月 - 1 月）/ 1 月）。

思路: CTE(group by) + 临时表，求多月结果集，搭配`CASE WHEN`进行环比计算。
...
- 数据源如何去重
- 上游的数据同步(尤其ODS到DWD)是否有延迟，延迟是否影响数据准确性，或者导致分子、分母数值为0，分母为0时给默认值还是异常提示
- 上下游表模型字段类型与长度是否统一
