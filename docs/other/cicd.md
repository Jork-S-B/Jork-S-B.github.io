...
```mermaid
flowchart LR
    A[合代码至主干] -->|触发Webhook| B[Jenkins拉代码]
    B --> D[Maven/Gradle构建<br>JUnit单元测试]
    D --> H[SonarQube扫描]
    F --> G[自动化测试<br>（API/E2E）]
    H -- 质量门禁未通过<br>（Bug/漏洞/覆盖率不足等） --> I[流水线中断/回滚]
    H -- 质量门禁通过 --> F[容器化部署到测试环境]  

```

SonarQube扫描覆盖的维度: 

- 代码“硬伤”与可靠性: 扫描可能导致程序崩溃、死循环或错误结果的逻辑缺陷。
- 安全漏洞与合规: 注入类、硬编码等。
- 代码异味与可维护性: 过长的函数、过大的类、复杂度过高的嵌套逻辑、未使用的变量、无效的导入声明、违反命名规范等。
- 单元测试与代码覆盖率
- 重复代码
- 多类型文件: 前后端文件、配置文件等。
- 开源依赖风险
