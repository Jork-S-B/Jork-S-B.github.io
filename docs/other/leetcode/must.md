...
=== "py"
    ...
    ```python
    # 取首个重复的char下标，dict手动计数或者 collections.Counter
...
1~100的列表，请你设计代码，取列表中的质数，并返回质数列表。

```python
# 小范围逐个试除，质数: 只能被1和本身整除的数
# 1. 分段处理，1不处理，2是质数
# 2. 3~100逐个试除并排除偶数，即每次迭代步长为2
# 3. 缩小范围可从end下手，只需要判断到 sqrt(end)
