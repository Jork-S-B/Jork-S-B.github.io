...
OOP 核心特性 + Python 特有机制

主要特性包括：封装、继承、多态、抽象；同时具备动态语言特性，如类也是对象、支持元类、多继承和 MRO(`super()`)、魔术方法/运算符重载、property/描述符、类方法/静态方法、实例属性与类属性、运行时动态修改类，以及鸭子类型。

1. 封装

- 把属性和方法包装在类里。
- Python 没有真正私有：`_x` 是约定保护，`__x` 会触发名称改写为 `_类名__x`。
- 可用 `property`、描述符控制属性访问。

2. 继承

- 支持单继承和多继承。
- 子类可复用、扩展、重写父类方法。
- 用 `super()` 按 MRO 调用父类方法。
- 多继承方法查找顺序由 C3 算法 MRO 决定：`类.__mro__`。

3. 多态

- 同一接口，不同实现；子类重写父类方法。
- Python 更强调 鸭子类型：不关心对象是什么类型，只关心它有没有对应方法/行为。

4. 抽象

- 通过 `abc.ABC`、`@abstractmethod` 定义抽象基类，约束子类实现。
- 也可用 `typing.Protocol` 做结构化子类型。

5. 类也是对象

- Python 中一切皆对象，类本身也是对象。
- 类通常由 `type` 创建；元类 `metaclass` 可控制类的创建过程。
- Python 3 所有类默认继承 `object`，都是新式类。

6. 类属性与实例属性

- 类属性属于类，所有实例共享。
- 实例属性属于具体实例，通常写在 `__init__` 里。
- 查找顺序大致：实例 `__dict__` → 类 `__dict__` → 父类 MRO。

7. 方法类型

- 实例方法：第一个参数 `self`。
- 类方法：`@classmethod`，第一个参数 `cls`。
- 静态方法：`@staticmethod`，无默认绑定参数。
...
- `__getitem__` / `__iter__` / `__next__`：容器、迭代协议。
...
- 运行时可以给类或实例动态添加、修改、删除属性/方法。
- 可以猴子补丁。
- 可以用 `type()` 动态创建类。
- `__slots__` 可限制实例属性、节省内存。

10. 属性管理

- `property`：getter/setter/deleter。
- 描述符：实现 `__get__`、`__set__`、`__delete__`，是 `property`、方法绑定等的基础。
- `__getattr__`、`__getattribute__` 可拦截属性访问。

11. 构造与析构

- `__new__` 负责创建，`__init__` 负责初始化。
- `__del__` 是析构方法，但不保证及时调用，不推荐依赖它做资源释放。
