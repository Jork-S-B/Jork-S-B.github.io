...
??? note "Service 类型详解"
    ...
    - ClusterIP（默认类型）
        ...
        作用范围: 仅在集群内部可访问
        访问方式: 其他 Pod 可以通过 Service 名称 + 端口 访问
        适用场景: 微服务之间内部通信
        示例: http://mock-service:80（只能在集群内其他 Pod 中使用）

    - NodePort

        作用范围: 集群内部 + 宿主机（Node）
        工作原理: 在集群的每个节点（Node）上开放一个固定端口（默认范围 30000-32767），外部可以通过 节点 IP:NodePort 访问服务
        适用场景: 需要从集群外部直接访问服务（如测试环境）
        示例: http://<Node-IP>:30080

    - LoadBalancer

        作用范围: 集群内部 + 外部（通过云厂商负载均衡器）
        工作原理: 在云环境（AWS、阿里云等）中会自动创建外部负载均衡器
        适用场景: 生产环境**对外**提供服务
        示例: http://localhost:<port>

    - ExternalName

        作用范围: DNS 别名
        工作原理: 将 Service 映射到一个外部域名
        适用场景: 将集群内服务指向外部数据库或 API
