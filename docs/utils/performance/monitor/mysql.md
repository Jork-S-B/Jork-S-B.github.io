...
问题现象: 压测流量归零后，服务器 CPU 正常，但内存占用持续高位不回落。

排查1: 通过 Arthas 的 dashboard 观察 JVM 资源。发现 Old 区占满却几乎没有 GC 活动（GC Count 和 GC Time 基本停滞）。  
基本排除“内存泄漏导致频繁 Full GC”的可能，怀疑焦点从‘对象创建过多’转移到‘线程阻塞导致对象无法释放’上。

排查2: thread -b 死锁检测，输出死锁线程 ID 和锁资源信息。结合调用栈，定位到根因是两处业务代码的加锁顺序颠倒（A方法先锁本地再锁分布式，B方法反之），导致线程互相死等。

处理与闭环: 推动统一加锁顺序，使用同样的压测模型做了回归复测。
