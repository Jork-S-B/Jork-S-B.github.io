...
??? note "note"
    ...
    arousal：n.唤起；兴奋；觉醒；  
    physiological arousal：指身体对于外界刺激的生理反应，如心率加快、血压升高、呼吸加深等。
...
with the performance of subjects who were required to monitor three _dials_ at a time, a
...
??? note "note"
    ...
    The UN warned the country not to interfere in its neighbor's internal affairs.（联合国警告该国不要干涉邻国内政。）

Probably the most significant finding from research on noise is that its predictability is
more important than how loud it is. We are much more able to 'tune out' _chronic_
...
??? note "note"
    ...
    fatigue：n.疲乏，厌倦；士兵杂役（尤指惩罚）；v.使疲劳，使劳累；adj.疲劳的
...
_transient_ effects were studied. But the major worry about noisy environments is that living 
...
??? note "note"
    ...
    transient：同`very short`；adj.转瞬即逝的；（工作）临时的；n.暂住者，流动人口；（电流、电压、频率的）瞬变
