...
1. 用户输入。
2. LLM 理解、规划、决策。
3. LLM 通过 Function Calling 输出结构化工具调用意图。
4. Agent Runtime 解析并执行。
5. 如果工具走 MCP，则由 MCP Client 按 MCP 协议请求 MCP Server。
6. MCP Server 执行工具/访问数据源，返回结果。
7. Runtime 把结果回填给 LLM。
8. LLM 继续推理，直到生成最终回答或完成任务。

总结：Function Calling 让模型决定“要调用什么工具”；MCP 让应用以标准方式“连接和调用外部工具”；Agent Runtime 是负责执行、编排和状态管理的“执行者”。

### CLI还是MCP？

CLI 和 MCP 的核心区别在于**服务对象**与**交互逻辑**
