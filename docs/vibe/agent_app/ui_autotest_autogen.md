...
基于AutoGen的多智能体协作平台，ui自动化的部分基于midscene。

以自然语言描述，midscene会驱动多模态模型规划并操作界面。

不足之处

1. 成本与稳定性: 每次执行都消耗token，由agent生成代码与执行，如何控制成本及保证稳定性
2. 断言，也是ui自动化agent应用的通病，简单的增删改通过count断言的，但复杂业务、中间件检查相关的断言如何生成
